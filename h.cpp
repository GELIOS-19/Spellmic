...
namespace mongo {
  ...
  namespace repl {
    namespace {
      ...
      void abortIndexBuilds(OperationContext* opCtx, const OplogEntry::CommandType& commandType, const NamespaceString& nss, const std::string& reason) {
        ...
        } else if (commandType == OplogEntry::CommandType::kDrop || commandType == OplogEntry::CommandType::kDropIndexes || commandType == OplogEntry::CommandType::kCollMod || commandType == OplogEntry::CommandType::kEmptyCapped || commandType == OplogEntry::CommandType::kRenameCollection) {
          const boost::optional<UUID> collUUID = CollectionCatalog::get(opCtx)->lookupUUIDByNSS(opCtx, nss);
          invariant(collUUID);
      ...
      void applyImportCollectionDefault(OperationContext* opCtx, const UUID& importUUID, const NamespaceString& nss, long long numRecords, long long dataSize, const BSONObj& catalogEntry, const BSONObj& storageMetadata, bool isDryRun, OplogApplication::Mode mode) {
        LOGV2_FATAL_NOTRACE(5114200, "Applying importCollection is not supported with MongoDB Community " "Edition, please use MongoDB Enterprise Edition", "importUUID"_attr = importUUID, logAttrs(nss), "numRecords"_attr = numRecords, "dataSize"_attr = dataSize, "catalogEntry"_attr = redact(catalogEntry), "storageMetadata"_attr = redact(storageMetadata), "isDryRun"_attr = isDryRun);
      }
    ...
    void createIndexForApplyOps(OperationContext* opCtx, const BSONObj& indexSpec, const NamespaceString& indexNss, OplogApplication::Mode mode) {
      ...
      auto indexCollection = db ? CollectionCatalog::get(opCtx)->lookupCollectionByNamespace(opCtx, indexNss) : nullptr;
      uassert(ErrorCodes::NamespaceNotFound, str::stream() << "Failed to create index due to missing collection: " << indexNss.ns(), indexCollection);
      ...
      if (OplogApplication::Mode::kInitialSync == mode) {
        auto normalSpecs = indexBuildsCoordinator->normalizeIndexSpecs(opCtx, indexCollection, {indexSpec});
        invariant(1U == normalSpecs.size(), str::stream() << "Unexpected result from normalizeIndexSpecs - ns: " << indexNss << "; uuid: " << indexCollection->uuid() << "; original index spec: " << indexSpec << "; normalized index specs: " << BSON("normalSpecs" << normalSpecs));
        ...
        if (ErrorCodes::IndexBuildAlreadyInProgress == prepareSpecResult) {
          LOGV2(4924900, "Index build: already in progress during initial sync", logAttrs(indexNss), "uuid"_attr = indexCollection->uuid(), "spec"_attr = indexSpec);
          return;
        }
      }
      ...
      invariant(ReplicationCoordinator::get(opCtx)->shouldRelaxIndexConstraints(opCtx, indexNss), str::stream() << "Unexpected result from shouldRelaxIndexConstraints - ns: " << indexNss << "; uuid: " << indexCollection->uuid() << "; original index spec: " << indexSpec);
     ...
     */
    void writeToImageCollection(OperationContext* opCtx, const LogicalSessionId& sessionId, const TxnNumber txnNum, const Timestamp timestamp, repl::RetryImageEnum imageKind, const BSONObj& dataImage, const StringData& invalidatedReason, bool* upsertConfigImage) {
      ...
      DisableDocumentValidation documentValidationDisabler(opCtx, DocumentValidationSettings::kDisableInternalValidation);

      UpdateRequest request;
      request.setNamespaceString(NamespaceString::kConfigImagesNamespace);
      request.setQuery(BSON("_id" << imageEntry.get_id().toBSON() << "ts" << BSON("$lt" << imageEntry.getTs())));
      request.setUpsert(*upsertConfigImage);
      request.setUpdateModification(write_ops::UpdateModification::parseFromClassicUpdate(imageEntry.toBSON()));
    ...
    */
     ...
     */
    void _logOpsInner(OperationContext* opCtx, const NamespaceString& nss, std::vector<Record>* records, const std::vector<Timestamp>& timestamps, const CollectionPtr& oplogCollection, OpTime finalOpTime, Date_t wallTime, bool isAbortIndexBuild) {
      auto replCoord = ReplicationCoordinator::get(opCtx);
      if (replCoord->getReplicationMode() == ReplicationCoordinator::modeReplSet && !replCoord->canAcceptWritesFor(opCtx, nss)) {
        str::stream ss;
      ...
      if (!result.isOK()) {
        LOGV2_FATAL(17322, "write to oplog failed: {error}", "Write to oplog failed", "error"_attr = result.toString());
      }
      ...
      opCtx->recoveryUnit()->onCommit(
        [opCtx, replCoord, finalOpTime, wallTime](boost::optional<Timestamp> commitTime) {
          if (commitTime) {
            ...
            invariant(finalOpTime.getTimestamp() <= *commitTime, str::stream() << "Final OpTime: " << finalOpTime.toString() << ". Commit Time: " << commitTime->toString());
          }
    ...
    OpTime logOp(OperationContext* opCtx, MutableOplogEntry* oplogEntry) {
      ...
      invariant(oplogEntry->getUuid() || oplogEntry->getOpType() == OpTypeEnum::kNoop || oplogEntry->getOpType() == OpTypeEnum::kCommand, str::stream() << "Expected uuid for logOp with oplog entry: " << redact(oplogEntry->toBSON()));
      ...
      if (replCoord->isOplogDisabledFor(opCtx, oplogEntry->getNss())) {
        uassert(ErrorCodes::IllegalOperation, str::stream() << "retryable writes is not supported for unreplicated ns: " << oplogEntry->getNss().ns(), oplogEntry->getStatementIds().empty());
        return {};
      }
      ...
      ScopeGuard resetOpTimeGuard(
        [&, resetOpTimeOnExit = bool(slot.isNull())] {
          if (resetOpTimeOnExit) oplogEntry->setOpTime(OplogSlot());
        });
      ...
      const auto isAbortIndexBuild = oplogEntry->getOpType() == OpTypeEnum::kCommand && parseCommandType(oplogEntry->getObject()) == OplogEntry::CommandType::kAbortIndexBuild;
      _logOpsInner(opCtx, oplogEntry->getNss(), &records, timestamps, oplog, slot, wallClockTime, isAbortIndexBuild);
      wuow.commit();
      return slot;
    }

    std::vector<OpTime> logInsertOps(OperationContext* opCtx, MutableOplogEntry* oplogEntryTemplate, std::vector<InsertStatement>::const_iterator begin, std::vector<InsertStatement>::const_iterator end, std::function<boost::optional<ShardId>(const BSONObj & doc)> getDestinedRecipientFn) {
      invariant(begin != end);
      ...
      if (replCoord->isOplogDisabledFor(opCtx, nss)) {
        ...
        uassert(ErrorCodes::IllegalOperation, str::stream() << "retryable writes is not supported for unreplicated ns: " << nss.ns(), begin->stmtIds.front() == kUninitializedStmtId);
        return {};
      }
      ...
      for (size_t i = 0; i < count; i++) {
        ...
        OplogLink oplogLink;
        if (i > 0) oplogLink.prevOpTime = opTimes[i - 1];
      ...
      _logOpsInner(opCtx, nss, &records, timestamps, oplog, lastOpTime, wallClockTime, isAbortIndexBuild);
      wuow.commit();
      return opTimes;
    }

    void appendOplogEntryChainInfo(OperationContext* opCtx, MutableOplogEntry* oplogEntry, OplogLink* oplogLink, const std::vector<StmtId>& stmtIds) {
    ...
    namespace {
      long long getNewOplogSizeBytes(OperationContext* opCtx, const ReplSettings& replSettings) {
        ...
        if (pi.getAddrSize() == 32) {
          ...
          LOGV2_DEBUG(21245, 3, "32bit system; choosing {oplogSizeBytes} bytes oplog", "Choosing oplog size for 32bit system", "oplogSizeBytes"_attr = sz);
          return sz;
        }
    ...
    return sz;
        ...
        if (opCtx->getClient()->getServiceContext()->getStorageEngine()->isEphemeral()) {
          ...
          LOGV2_DEBUG(21247, 3, "Ephemeral storage system; lowerBound: {lowerBoundBytes} bytes, " "{totalMemoryBytes} bytes total memory", "Ephemeral storage system", "lowerBoundBytes"_attr = lowerBound, "totalMemoryBytes"_attr = bytes);
        } else {
          // disk: 990MB minimum size
          lowerBound = 990LL * 1024 * 1024;
          bytes = File::freeSpace(storageGlobalParams.dbpath); //-1 if call not supported.
          LOGV2_DEBUG(21248, 3, "Disk storage system; lowerBound: {lowerBoundBytes} bytes, {freeSpaceBytes} " "bytes free space " "on device", "Disk storage system", "lowerBoundBytes"_attr = lowerBound, "freeSpaceBytes"_attr = bytes);
        }
    ...
    void createOplog(OperationContext* opCtx, const NamespaceString& oplogCollectionName, bool isReplSet) {
      ...
      CollectionPtr collection = CollectionCatalog::get(opCtx)->lookupCollectionByNamespace(opCtx, oplogCollectionName);
      ...
      if (collection) {
        if (replSettings.getOplogSizeBytes() != 0) {
          ...
          if (n != o) {
            static constexpr char message[] = "Command line oplog size different than existing. See " "http://dochub.mongodb.org/core/increase-oplog";
            LOGV2(21249, message, "commandLineOplogSize"_attr = n, "existingOplogSize"_attr = o);
            uasserted(13257, str::stream() << message << ". Command line oplog size: " << n << ", existing oplog size: " << o);
          }
        }
        acquireOplogCollectionForLogging(opCtx);
        if (!isReplSet) initTimestampFromOplog(opCtx, oplogCollectionName);
        return;
      }
      ...
      LOGV2(21251, "creating replication oplog of size: {oplogSizeMegabytes}MB...", "Creating replication oplog", "oplogSizeMB"_attr = (int)(sz / (1024 * 1024)));
      ...
      writeConflictRetry(
        opCtx, "createCollection", oplogCollectionName.ns(), [&] {
    ...
    void createOplog(OperationContext* opCtx) {
      const auto isReplSet = ReplicationCoordinator::get(opCtx)->getReplicationMode() == ReplicationCoordinator::modeReplSet;
    ...
    namespace {
      NamespaceString extractNs(StringData db, const BSONObj& cmdObj) {
        ...
        uassert(40073, str::stream() << "collection name has invalid type " << typeName(first.type()), first.canonicalType() == canonicalizeBSONType(mongo::String));
      ...
      NamespaceString extractNsFromUUIDorNs(OperationContext* opCtx, const NamespaceString& ns, const boost::optional<UUID>& ui, const BSONObj& cmd) {
        return ui ? extractNsFromUUID(opCtx, ui.get()) : extractNs(ns.db(), cmd);
      }

      using OpApplyFn = std::function<Status(OperationContext * opCtx, const OplogEntry & entry, OplogApplication::Mode mode)>;
      ...
      const StringMap<ApplyOpMetadata> kOpsMap = {
          ...
          "create", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              ...
              if (auto idIndexElem = cmd["idIndex"]) {
                ...
                return createCollectionForApplyOps(opCtx, nss.db().toString(), ui, cmdWithoutIdIndex, allowRenameOutOfTheWay, idIndexElem.Obj());
              }

              // Collections clustered by _id do not need _id indexes.
              if (auto clusteredElem = cmd["clusteredIndex"]) {
                return createCollectionForApplyOps(opCtx, nss.db().toString(), ui, cmd, allowRenameOutOfTheWay, boost::none);
              }

              // No _id index spec was provided, so we should build a v:1 _id index.
              BSONObjBuilder idIndexSpecBuilder;
              idIndexSpecBuilder.append(IndexDescriptor::kIndexVersionFieldName, static_cast<int>(IndexVersion::kV1));
              ...
              return createCollectionForApplyOps(opCtx, nss.db().toString(), ui, cmd, allowRenameOutOfTheWay, idIndexSpecBuilder.done());
            },
          ...
          "createIndexes", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              ...
              if (OplogApplication::Mode::kApplyOpsCmd == mode) {
                return {ErrorCodes::CommandNotSupported, "The createIndexes operation is not supported in applyOps mode"};
              }

              const NamespaceString nss(extractNsFromUUIDorNs(opCtx, entry.getNss(), entry.getUuid(), cmd));
              BSONElement first = cmd.firstElement();
              invariant(first.fieldNameStringData() == "createIndexes");
              uassert(ErrorCodes::InvalidNamespace, "createIndexes value must be a string", first.type() == mongo::String);
            ...
            {ErrorCodes::IndexAlreadyExists, ErrorCodes::IndexBuildAlreadyInProgress, ErrorCodes::NamespaceNotFound}
          }
        },
        {
          "startIndexBuild", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              if (OplogApplication::Mode::kApplyOpsCmd == mode) {
                return {ErrorCodes::CommandNotSupported, "The startIndexBuild operation is not supported in applyOps mode"};
              }

              auto swOplogEntry = IndexBuildOplogEntry::parse(entry);
              if (!swOplogEntry.isOK()) {
                return swOplogEntry.getStatus().withContext("Error parsing 'startIndexBuild' oplog entry");
              }

              IndexBuildsCoordinator::ApplicationMode applicationMode = IndexBuildsCoordinator::ApplicationMode::kNormal;
              if (mode == OplogApplication::Mode::kInitialSync) {
                applicationMode = IndexBuildsCoordinator::ApplicationMode::kInitialSync;
              }
              IndexBuildsCoordinator::get(opCtx)->applyStartIndexBuild(opCtx, applicationMode, swOplogEntry.getValue());
              return Status::OK();
            },
            {ErrorCodes::IndexAlreadyExists, ErrorCodes::IndexBuildAlreadyInProgress, ErrorCodes::NamespaceNotFound}
          }
        },
        {
          "commitIndexBuild", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              if (OplogApplication::Mode::kApplyOpsCmd == mode) {
                return {ErrorCodes::CommandNotSupported, "The commitIndexBuild operation is not supported in applyOps mode"};
              }

              auto swOplogEntry = IndexBuildOplogEntry::parse(entry);
              if (!swOplogEntry.isOK()) {
                return swOplogEntry.getStatus().withContext("Error parsing 'commitIndexBuild' oplog entry");
              }
              auto* indexBuildsCoordinator = IndexBuildsCoordinator::get(opCtx);
              indexBuildsCoordinator->applyCommitIndexBuild(opCtx, swOplogEntry.getValue());
              return Status::OK();
            },
            {ErrorCodes::IndexAlreadyExists, ErrorCodes::IndexBuildAlreadyInProgress, ErrorCodes::NamespaceNotFound, ErrorCodes::NoSuchKey}
          }
        },
        {
          "abortIndexBuild", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              if (OplogApplication::Mode::kApplyOpsCmd == mode) {
                return {ErrorCodes::CommandNotSupported, "The abortIndexBuild operation is not supported in applyOps mode"};
              }

              auto swOplogEntry = IndexBuildOplogEntry::parse(entry);
              if (!swOplogEntry.isOK()) {
                return swOplogEntry.getStatus().withContext("Error parsing 'abortIndexBuild' oplog entry");
              }
          ...
          "collMod", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              ...
              const auto nssOrUUID(
                [&collModCmd, &entry, mode]() -> NamespaceStringOrUUID {
          ...
          "drop", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              ...
              if (nss.isDropPendingNamespace()) {
                LOGV2(21253, "applyCommand: {namespace} : collection is already in a drop-pending state: " "ignoring collection drop: {command}", "applyCommand: collection is already in a drop-pending state, ignoring " "collection drop", "namespace"_attr = nss, "command"_attr = redact(cmd));
                return Status::OK();
              }
              ...
              return dropCollectionForApplyOps(opCtx, nss, opTime, DropCollectionSystemCollectionMode::kAllowSystemCollectionDrops);
            },
            {ErrorCodes::NamespaceNotFound}
          }
        },
        // deleteIndex(es) is deprecated but still works as of April 10, 2015
        {
          "deleteIndex", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              const auto& cmd = entry.getObject();
              return dropIndexesForApplyOps(opCtx, extractNsFromUUID(opCtx, entry.getUuid().get()), cmd);
            },
        ...
        },
        {
          "renameCollection", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              ...
              return renameCollectionForApplyOps(opCtx, entry.getNss().db().toString(), entry.getUuid(), entry.getObject(), opTime);
            },
        ...
        },
        {
          "importCollection", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              auto importEntry = mongo::ImportCollectionOplogEntry::parse(IDLParserErrorContext("importCollectionOplogEntry"), entry.getObject());
              applyImportCollection(opCtx, importEntry.getImportUUID(), importEntry.getImportCollection(), importEntry.getNumRecords(), importEntry.getDataSize(), importEntry.getCatalogEntry(), importEntry.getStorageMetadata(), importEntry.getDryRun(), mode);
              return Status::OK();
            },
            {ErrorCodes::NamespaceExists}
          }
        ...
        },
        {
          "convertToCapped", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              const auto& cmd = entry.getObject();
              convertToCapped(opCtx, extractNsFromUUIDorNs(opCtx, entry.getNss(), entry.getUuid(), cmd), cmd["size"].number());
              return Status::OK();
            },
            {ErrorCodes::NamespaceNotFound}
          }
        },
        {
          "emptycapped", {
            [](OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) -> Status {
              return emptyCapped(opCtx, extractNsFromUUIDorNs(opCtx, entry.getNss(), entry.getUuid(), entry.getObject()));
            },
            {ErrorCodes::NamespaceNotFound}
          }
        },
      ...
      void writeChangeStreamPreImage(OperationContext* opCtx, const CollectionPtr& collection, const mongo::repl::OplogEntry& oplogEntry, const BSONObj& preImage) {
        ChangeStreamPreImageId preImageId{collection->uuid(), oplogEntry.getTimestamp(), 0 /*applyOpsIndex*/};
        ChangeStreamPreImage preImageDocument{std::move(preImageId), oplogEntry.getWallClockTime(), preImage};
    ...
    StringData OplogApplication::modeToString(OplogApplication::Mode mode) {
      switch (mode) {
      case OplogApplication::Mode::kInitialSync: return OplogApplication::kInitialSyncOplogApplicationMode;
      case OplogApplication::Mode::kRecovering: return OplogApplication::kRecoveringOplogApplicationMode;
      case OplogApplication::Mode::kSecondary: return OplogApplication::kSecondaryOplogApplicationMode;
      case OplogApplication::Mode::kApplyOpsCmd: return OplogApplication::kApplyOpsCmdOplogApplicationMode;
      }
      MONGO_UNREACHABLE;
    }
    ...
    StatusWith<OplogApplication::Mode> OplogApplication::parseMode(const std::string& mode) {
      if (mode == OplogApplication::kInitialSyncOplogApplicationMode) {
        ...
        return Status(ErrorCodes::FailedToParse, str::stream() << "Invalid oplog application mode provided: " << mode);
      }
      MONGO_UNREACHABLE;
    }
    ...
    Status applyOperation_inlock(OperationContext* opCtx, Database* db, const OplogEntryOrGroupedInserts& opOrGroupedInserts, bool alwaysUpsert, OplogApplication::Mode mode, const bool isDataConsistent, IncrementOpsAppliedStatsFn incrementOpsAppliedStats) {
      // Get the single oplog entry to be applied or the first oplog entry of grouped inserts.
      auto op = opOrGroupedInserts.getOp();
      LOGV2_DEBUG(21254, 3, "applying op (or grouped inserts): {op}, oplog application mode: " "{oplogApplicationMode}", "Applying op (or grouped inserts)", "op"_attr = redact(opOrGroupedInserts.toBSON()), "oplogApplicationMode"_attr = OplogApplication::modeToString(mode));
      ...
      const bool shouldUseGlobalOpCounters = mode == repl::OplogApplication::Mode::kApplyOpsCmd || opCtx->writesAreReplicated();
      ...
      if (auto uuid = op.getUuid()) {
        ...
        uassert(ErrorCodes::NamespaceNotFound, str::stream() << "Failed to apply operation due to missing collection (" << uuid.get() << "): " << redact(opOrGroupedInserts.toBSON()), collection);
        ...
        dassert(opCtx->lockState()->isCollectionLockedForMode(requestNss, MODE_IX), requestNss.ns());
      ...
      // during initial sync.
      if ((mode == OplogApplication::Mode::kInitialSync) && requestNss == NamespaceString::kServerConfigurationNamespace) {
        std::string oID;
        auto status = bsonExtractStringField(o, "_id", &oID);
        if (status.isOK() && oID == multiversion::kParameterName) {
          return Status(ErrorCodes::OplogOperationUnsupported, str::stream() << "Applying operation on feature compatibility version " "document not supported in initial sync: " << redact(opOrGroupedInserts.toBSON()));
        }
      }

      BSONObj o2;
      if (op.getObject2()) o2 = op.getObject2().get();
      ...
      uassert(ErrorCodes::CommandNotSupportedOnView, str::stream() << "applyOps not supported on view: " << requestNss.ns(), collection || !ViewCatalog::get(db)->lookup(opCtx, requestNss));
      ...
      invariant(!assignOperationTimestamp || !op.getTimestamp().isNull(), str::stream() << "Oplog entry did not have 'ts' field when expected: " << redact(opOrGroupedInserts.toBSON()));
      ...
      auto shouldRecordChangeStreamPreImage = [&]() {
        ...
        return collection && collection->isChangeStreamPreAndPostImagesEnabled() && isDataConsistent && (mode == OplogApplication::Mode::kRecovering || mode == OplogApplication::Mode::kSecondary) && !op.getFromMigrate().get_value_or(false) && !requestNss.isTemporaryReshardingCollection();
      };

      switch (opType) {
      case OpTypeEnum::kInsert: {
        uassert(ErrorCodes::NamespaceNotFound, str::stream() << "Failed to apply insert due to missing collection: " << redact(opOrGroupedInserts.toBSON()), collection);
        if (opOrGroupedInserts.isGroupedInserts()) {
          ...
          uassert(ErrorCodes::OperationFailed, "Cannot apply an array insert with applyOps", !opCtx->writesAreReplicated() || tenantMigrationRecipientInfo(opCtx));
          ...
          if (!opCtx->writesAreReplicated()) {
            for (const auto iOp : insertOps) {
              ...
              insertObjs.emplace_back(iOp->getObject(), iOp->getTimestamp(), iOp->getTerm().get());
            }
          } else {
            ...
            for (const auto iOp : insertOps) {
              insertObjs.emplace_back(iOp->getObject(), slotIter->getTimestamp(), slotIter->getTerm());
              slotIter++;
            }
          }

          OpDebug* const nullOpDebug = nullptr;
          Status status = collection->insertDocuments(opCtx, insertObjs.begin(), insertObjs.end(), nullOpDebug, false /* fromMigrate */);
          if (!status.isOK()) {
            return status;
          }
          wuow.commit();
          ...
          uassert(ErrorCodes::NoSuchKey, str::stream() << "Failed to apply insert due to missing _id: " << redact(op.toBSONForLogging()), o.hasField("_id"));
          ...
          if (!needToDoUpsert) {
            ...
            } else if (!repl::ReplicationCoordinator::get(opCtx)->isOplogDisabledFor(opCtx, collection->ns())) {
            ...
            OpDebug* const nullOpDebug = nullptr;
            Status status = collection->insertDocument(opCtx, insertStmt, nullOpDebug, false /* fromMigrate */);
          ...
          if (needToDoUpsert) {
            ...
            request.setUpdateModification(write_ops::UpdateModification::parseFromClassicUpdate(o));
            request.setUpsert();
            request.setFromOplogApplication(true);

            const StringData ns = op.getNss().ns();
            writeConflictRetry(
              opCtx, "applyOps_upsert", ns, [&] {
                WriteUnitOfWork wuow(opCtx);
                ...
                if (res.numMatched == 0 && res.upsertedId.isEmpty()) {
                  LOGV2_ERROR(21257, "No document was updated even though we got a DuplicateKey " "error when inserting");
      ...
      case OpTypeEnum::kUpdate: {
        ...
        auto idField = o2["_id"];
        uassert(ErrorCodes::NoSuchKey, str::stream() << "Failed to apply update due to missing _id: " << redact(op.toBSONForLogging()), !idField.eoo());
        ...
        if (mode == OplogApplication::Mode::kSecondary && collection->getTimeseriesOptions() && !op.getFromTenantMigration()) {
        ...
        auto status = writeConflictRetry(
          opCtx, "applyOps_update", ns, [&] {
            ...
            if (recordChangeStreamPreImage && request.shouldReturnNewDocs()) {
              ...
              // component.
              auto documentFound = Helpers::findOne(opCtx, collection, documentId, changeStreamPreImage, requireIndex);
              invariant(documentFound);
            }

            UpdateResult ur = update(opCtx, db, request);
            if (ur.numMatched == 0 && ur.upsertedId.isEmpty()) {
              if (collection && collection->isCapped() && mode == OplogApplication::Mode::kSecondary) {
                ...
                LOGV2_DEBUG(2170003, 2, "couldn't find doc in capped collection", "op"_attr = redact(op.toBSONForLogging()));
                ...
                if (updateCriteria.nFields() == 1) {
                  ...
                  return Status(ErrorCodes::UpdateOperationFailed, str::stream() << msg << ": " << redact(op.toBSONForLogging()));
                }
                ...
                if (collection == nullptr || (indexCatalog->haveIdIndex(opCtx) && Helpers::findById(opCtx, collection, updateCriteria).isNull()) ||
                  // capped collections won't have an _id index
                  (!indexCatalog->haveIdIndex(opCtx) && Helpers::findOne(opCtx, collection, updateCriteria, false).isNull())) {
            ...
            } else if (mode == OplogApplication::Mode::kSecondary && !upsertOplogEntry && !ur.upsertedId.isEmpty() && !(collection && collection->isCapped())) {
              // This indicates we upconverted an update to an upsert, and it did indeed
              // upsert.  In steady state mode this is unexpected.
              LOGV2_WARNING(2170001, "update needed to be converted to upsert", "op"_attr = redact(op.toBSONForLogging()));
            ...
            if (op.getNeedsRetryImage()) {
              writeToImageCollection(
                opCtx, op.getSessionId().get(), op.getTxnNumber().get(), op.getTimestampForRetryImage().value_or(op.getTimestamp()), op.getNeedsRetryImage().get(),
                // If we did not request an image because we're in
                // initial sync, the value passed in here is conveniently
                // the empty BSONObj.
                ur.requestedDocImage, getInvalidatingReason(mode, isDataConsistent), &upsertConfigImage);
            }
      ...
      case OpTypeEnum::kDelete: {
        ...
        auto idField = o["_id"];
        uassert(ErrorCodes::NoSuchKey, str::stream() << "Failed to apply delete due to missing _id: " << redact(op.toBSONForLogging()), !idField.eoo());
        ...
        writeConflictRetry(
          opCtx, "applyOps_delete", ns, [&] {
            ...
            if (mode != OplogApplication::Mode::kInitialSync && op.getNeedsRetryImage() == repl::RetryImageEnum::kPreImage && isDataConsistent) {
            ...
            if (op.getNeedsRetryImage()) {
              ...
              writeToImageCollection(opCtx, op.getSessionId().get(), op.getTxnNumber().get(), op.getTimestampForRetryImage().value_or(op.getTimestamp()), repl::RetryImageEnum::kPreImage, result.requestedPreImage.value_or(BSONObj()), getInvalidatingReason(mode, isDataConsistent), &upsertConfigImage);
            }
            ...
            if (result.nDeleted == 0 && mode == OplogApplication::Mode::kSecondary) {
              LOGV2_WARNING(2170002, "Applied a delete which did not delete anything in steady state " "replication", "op"_attr = redact(op.toBSONForLogging()));
              ...
              if (!isCapped) {
                ...
                uassert(collection ? ErrorCodes::NoSuchKey : ErrorCodes::NamespaceNotFound, str::stream() << "Applied a delete which did not delete anything in " "steady state replication : " << redact(op.toBSONForLogging()), !oplogApplicationEnforcesSteadyStateConstraints);
              }
            }
            wuow.commit();
          });
    ...
    Status applyCommand_inlock(OperationContext* opCtx, const OplogEntry& entry, OplogApplication::Mode mode) {
      LOGV2_DEBUG(21255, 3, "applying command op: {oplogEntry}, oplog application mode: " "{oplogApplicationMode}", "Applying command op", "oplogEntry"_attr = redact(entry.toBSONForLogging()), "oplogApplicationMode"_attr = OplogApplication::modeToString(mode));
      ...
      }
      {
        ...
        if (db && !CollectionCatalog::get(opCtx)->lookupCollectionByNamespace(opCtx, nss) && ViewCatalog::get(db)->lookup(opCtx, nss)) {
          return {ErrorCodes::CommandNotSupportedOnView, str::stream() << "applyOps not supported on view:" << nss.ns()};
        }
      }
      ...
      std::vector<std::string> allowlistedOps{"dropDatabase", "applyOps", "dbCheck", "commitTransaction", "abortTransaction", "startIndexBuild", "commitIndexBuild", "abortIndexBuild"};
      if ((mode == OplogApplication::Mode::kInitialSync) && (std::find(allowlistedOps.begin(), allowlistedOps.end(), o.firstElementFieldName()) == allowlistedOps.end()) && extractNs(nss.db(), o) == NamespaceString::kServerConfigurationNamespace) {
        return Status(ErrorCodes::OplogOperationUnsupported, str::stream() << "Applying command to feature compatibility version " "collection not supported in initial sync: " << redact(entry.toBSONForLogging()));
      }
      ...
      const bool assignCommandTimestamp = [&] {
        ...
        if (entry.shouldPrepare() || entry.getCommandType() == OplogEntry::CommandType::kCommitTransaction || entry.getCommandType() == OplogEntry::CommandType::kAbortTransaction) return false;
      ...
      invariant(!assignCommandTimestamp || !opTime.isNull(), str::stream() << "Oplog entry did not have 'ts' field when expected: " << redact(entry.toBSONForLogging()));
      ...
      while (!done) {
        ...
        if (op == kOpsMap.end()) {
          return Status(ErrorCodes::BadValue, str::stream() << "Invalid key '" << o.firstElementFieldName() << "' found in field 'o'");
        }
        ...
        case ErrorCodes::BackgroundOperationInProgressForDatabase: {
          ...
          abortIndexBuilds(opCtx, entry.getCommandType(), nss, "Aborting index builds during initial sync");
          LOGV2_DEBUG(4665900, 1, "Conflicting DDL operation encountered during initial sync; " "aborting index build and retrying", "db"_attr = nss.db());
          break;
        }
        case ErrorCodes::BackgroundOperationInProgressForNamespace: {
          ...
          invariant(mode == OplogApplication::Mode::kInitialSync);
          abortIndexBuilds(opCtx, entry.getCommandType(), ns, "Aborting index builds during initial sync");
          LOGV2_DEBUG(4665901, 1, "Conflicting DDL operation encountered during initial sync; " "aborting index build and retrying", "namespace"_attr = ns);
        ...
        default: {
          ...
          if ((mode == OplogApplication::Mode::kSecondary && oplogApplicationEnforcesSteadyStateConstraints && status.code() != ErrorCodes::IndexNotFound && op->first != "dropDatabase") || !curOpToApply.acceptableErrors.count(status.code())) {
            LOGV2_ERROR(21262, "Failed command {command} on {db} with status {error} during oplog " "application", "Failed command during oplog application", "command"_attr = redact(o), "db"_attr = nss.db(), "error"_attr = status);
            return status;
          }

          if (mode == OplogApplication::Mode::kSecondary && status.code() != ErrorCodes::IndexNotFound) {
            LOGV2_WARNING(2170000, "Acceptable error during oplog application", "db"_attr = nss.db(), "error"_attr = status, "oplogEntry"_attr = redact(entry.toBSONForLogging()));
            opCounters->gotAcceptableErrorInCommand();
          } else {
            LOGV2_DEBUG(51776, 1, "Acceptable error during oplog application", "db"_attr = nss.db(), "error"_attr = status, "oplogEntry"_attr = redact(entry.toBSONForLogging()));
          }
        }
          // fallthrough
        case ErrorCodes::OK: done = true;
          break;
        }
      }
    ...
    void initTimestampFromOplog(OperationContext* opCtx, const NamespaceString& oplogNss) {
      ...
      BSONObj lastOp = c.findOne(std::move(findCmd), ReadPreferenceSetting{ReadPreference::SecondaryPreferred});
